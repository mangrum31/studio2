...
export default function WordWeaversPage() {
  ...
  function canFormWord(word: string) {
  ...
  return (
    <>
      ...
      `}</style>
      <div className="w-full max-w-4xl bg-card shadow-2xl rounded-xl p-6 sm:p-10 transition-all duration-300 mx-auto">
        ...
        <div className="flex justify-between items-center bg-primary/10 p-4 rounded-lg mb-6">
          ...
          <Button
            onClick={initGame}
            className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-full shadow-lg transition duration-150 transform hover:scale-105"
          >
            New Scramble
          </Button>
        </div>
        ...
        >
          {currentLetters.map((letter, i) => (
            <div
              key={i}
              className="letter-tile bg-yellow-300 text-yellow-900 rounded-xl flex items-center justify-center border-b-4 border-yellow-500"
            >
              {letter}
            </div>
          ))}
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4">
          <Input
            ...
            className="flex-grow p-4 border-2 border-primary/50 rounded-xl text-xl font-medium focus:outline-none focus:border-primary transition duration-150 uppercase"
            maxLength={10}
            autoComplete="off"
            value={inputValue}
        ...
        <div
          className={`h-8 text-center text-lg font-semibold mb-6 ${getMessageColor()}`}
        >
          {message?.text}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-muted/50 p-4 rounded-lg border border-border">
            ...
            <div
              id="found-words"
              className="h-64 overflow-y-auto p-2 bg-background rounded border border-dashed border-border"
            >
              {foundWords.size === 0 ? (
                ...
                sortedWords.map((word) => (
                  <span
                    key={word}
                    className="inline-block bg-primary/10 text-primary-foreground/80 text-sm font-medium px-3 py-1 rounded-full m-1 shadow-sm"
                  >
                    {word} ({word.length})
                  </span>
                ))
          ...
          <div className="lg:col-span-1 bg-muted/50 p-4 rounded-lg border border-border">
            <h2 className="text-xl font-semibold text-foreground mb-3">
              Scoring
            </h2>
